...
namespace Vulkan
{
	...
	void Pipeline::createGraphicsPipeline(const std::string& vertFilePath, 
										  ...
										  const PipelineConfigInfo& configInfo)
	{
		assert(configInfo.pipelineLayout != VK_NULL_HANDLE && "Cannot create graphics pipeline -> no pipelineLayout is provided in configInfo! ");
		assert(configInfo.renderPass != VK_NULL_HANDLE && "Cannot create graphics pipeline -> no renderPass is provided in configInfo! ");
		...
		auto& bindingDescription = configInfo.bindingDescription;
		auto& attributeDescription = configInfo.attributeDescription;
